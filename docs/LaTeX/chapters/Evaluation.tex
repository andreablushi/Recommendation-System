...
\\\\
Overall, the results indicate that the limited size of the test dataset, influence the number of the recommendation and so their statistics.
\\\\
For $\textit{prefix\_length}=5$, we can see as we achieve $0\%$ across all metrics, and all the few recommendations generated were considered as not followed.
...
Then they consistently failed to align with the actual trace outcomes.
...
Then the higher number of recommendations followed indicates that the system is better able to suggest changes that align with actual successful outcomes in the traces.
