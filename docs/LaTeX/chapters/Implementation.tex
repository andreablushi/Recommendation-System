...
This weighting incorporates the number of samples reaching each leaf node, thereby penalizing paths that achieve high confidence based on only a small number of observations.
...
A path is defined as compliant if none of its feature conditions are violated along the path by the activities present in the trace prefix.
